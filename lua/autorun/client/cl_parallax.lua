			...
			if count > 0 then
			    tabs.Add("Jobs", F4menu.configuration.tabs.jobs.color, function(p)
			    	...
			    	function F4menu.BuildChosen(v)
				    	...
				    	local mdl = (isstring(preferredModel) and preferredModel) or (istable(v.model) and v.model[1]) or (v.model)
				    	chosen.model:SetModel(mdl)
						...
						if chosen.model and isnumber(bonepos) then
						...
						if (istable(v.model) and (#v.model > 1)) and (chosen.model) then
			    	...
			    	for k, v in pairs(DarkRP.getCategories()["entities"]) do
			    		...
			    		for k, v in pairs(v.members) do
			    			...
			    			function entity:Paint(w, h)
								...
								if (not theme().light) or (cantbuy) then
									surface.SetDrawColor(0, 0, 0, 50)
				    				surface.DrawRect(0, 0, w, h)
				    			end
			    				...
			    				surface.SetFont("dev button")
			    				local gmax = v.getMax and v.getMax(LocalPlayer())
			    				local text
			    				if isnumber(gmax) then
			    					text = "Maximum " .. (isnumber(gmax) and gmax) or v.max
			    				elseif v.max then
			    					text = "Maximum " .. v.max
			    				else
			    					text = ""
